...
class MotionLibBase():
        ...
        if self.mode == MotionlibMode.file:
            if min_length != -1:
                ...
                data_list = {item[0]: item[1] for item in
                             sorted(self._motion_data_load.items(), key=lambda entry: len(entry[1]['pose_quat_global']), reverse=True)}
            else:
                data_list = self._motion_data_load
    ...
    def load_motions(self,
        ...
        # assert (sample_idxes == 0).all(), "Not Allowed to load more than one motion!"  # 临时注释，允许多运动
