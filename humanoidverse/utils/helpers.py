...
def pre_process_config(config) -> None:
    ...
    if config.log_task_name=='motion_tracking':
        ...
        if os.path.isfile(motion_file):
            ...
            # 支持float类型的fps，转换为int
            fps = the_motion_data['fps']
            # 如果是 numpy 数组，提取标量值
            if hasattr(fps, 'item'):  # numpy array or scalar
                fps = fps.item()
            elif hasattr(fps, '__iter__') and not isinstance(fps, str):  # 其他可迭代类型
                fps = fps[0] if len(fps) > 0 else 30  # 默认值
            
            if isinstance(fps, float):
                fps = int(round(fps))  # 四舍五入转换为整数
            elif not isinstance(fps, int):
                raise ValueError(f"FPS must be int or float, got {type(fps)}")
            
            the_motion_data['fps'] = fps  # 确保fps是整数类型
